    ...
    return declare(BaseWidget, {
      ...
      startup: function () {
        ...
        */
        ...
        connect.connect(this.map.infoWindow, 'onSetFeatures', function () {  // TODO make this a separate function called from also the event of click next popup and return from related table view
          
          if (services && Object.values(services) ){
            ...
            for (let LayerToAttachId of Object.values(services)) {
              ...
              if (featureLayer && featureLayer.hasAttachments) {
                ...
                for (var i = 0; fieldlist.length; i++) { 
                  if (fieldlist[i].type === 'esriFieldTypeOID') {
                    var idfield = fieldlist[i].name
                    break
                  } else {
                    console.log('no suitable id found in layer')
                  }
                }
                ...
                featureLayer.queryAttachmentInfos(attributeData.attributes[idfield], function (infos) {
                  ...
                  console.log("location of popup",popupLocation)
                  
                  
                  if ($('.contentPane').find('#initialImageDiv').length == 0) {
                    console.log ("BACK DAT ASSSS",$('.contentPane').find('.initialImageDiv').length)
                    if (popupLocation == 1) {
